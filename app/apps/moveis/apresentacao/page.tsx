...
const reviews = [
  { name: "Carlos Mendes", initials: "CM", profile: "Marceneiro autônomo", title: "Praticidade no dia a dia", text: "O que mais gostei foi a praticidade. Em poucos minutos consegui cadastrar o cliente, montar o orçamento e gerar um PDF muito mais apresentável." },
  { name: "Juliano Ribeiro", initials: "JR", profile: "Dono de marcenaria", title: "Um PDF profissional", text: "O PDF ficou com aparência profissional de verdade. Para quem trabalha com móveis sob medida, isso passa muito mais confiança para o cliente." },
  { name: "Rafael Costa", initials: "RC", profile: "Marceneiro e montador", title: "Simples de usar", text: "Gostei porque o sistema é direto. Não fica cheio de coisa desnecessária. Dá para usar no celular sem complicação e organizar os orçamentos com facilidade." },
  { name: "André Ferreira", initials: "AF", profile: "Pequena marcenaria", title: "A marca da minha marcenaria", text: "A parte de personalizar a marca é um diferencial muito bom. As cores e o layout deixam a proposta mais bonita e valorizam o meu trabalho." },
  { name: "Diego Martins", initials: "DM", profile: "Profissional de móveis planejados", title: "Tudo mais organizado", text: "Para quem fazia tudo no improviso, o OrçaMóvel ajuda bastante. Cliente, orçamento e PDF ficam no mesmo lugar, o que economiza tempo e evita retrabalho." },
];
const benefits = [
      ...
      <main id="conteudo">
          ...
          <div className={styles.container}><p className={styles.eyebrow}>PRIMEIRAS IMPRESSÕES</p><h2 id="avaliacoes">Avaliações iniciais</h2><p className={styles.reviewNote}>Exemplos ilustrativos de feedbacks de teste. Nomes e avatares fictícios; não são avaliações públicas verificadas.</p>
            <div className={styles.reviews} tabIndex={0} role="region" aria-label="Exemplos de feedbacks de teste">
              {reviews.map(review => <article key={review.name} className={styles.review}>
                <div className={styles.reviewer}><span className={styles.avatar} aria-hidden="true">{review.initials}</span><div><h3>{review.name}</h3><p>{review.profile}</p></div></div>
        ...
        <section id="planos" className={`${styles.container} ${styles.section}`}>
          ...
          <div className={styles.plans}>
            <article><p className={styles.planName}>Mensal</p><p className={styles.price}>R$ 9,99</p><p>30 dias de acesso</p><ul><li><Check size={17} />Todos os recursos</li><li><Check size={17} />Pagamento único por período</li></ul><Link href="/apps/moveis" className={styles.outlineButton}>Testar grátis <ArrowRight size={17} /></Link></article>
            <article className={styles.featuredPlan}><p className={styles.planName}>Anual</p><p className={styles.price}>R$ 99,99</p><p>365 dias de acesso</p><ul><li><Check size={17} />Todos os recursos</li><li><Check size={17} />Pagamento único por período</li></ul><Link href="/apps/moveis" className={styles.button}>Testar grátis <ArrowRight size={17} /></Link></article>
            <article><p className={styles.planName}>Vitalício</p><p className={styles.lifetimePrice}>Acesso sem vencimento</p><p>Consulte a oferta vigente no app</p><ul><li><Check size={17} />Todos os recursos</li><li><Check size={17} />Pagamento único</li></ul><Link href="/apps/moveis" className={styles.outlineButton}>Ver oferta no app <ArrowRight size={17} /></Link></article>
