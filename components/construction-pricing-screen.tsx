...
export function ConstructionPricingScreen({ email, onSignOut, onRefresh, onBack, trialEnded = true }: { email: string; onSignOut: () => void; onRefresh: () => void; onBack?: () => void; trialEnded?: boolean }) {
  ...
  const loadCurrentPlan = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select("subscription_status,plan_type,access_expires_at")
      .eq("user_id", user.id)
  ...
  useEffect(() => {
    const supabase = getSupabaseBrowserClient();
    if (supabase) {
      void supabase.rpc("get_orcamovel_settings").then(({ data }) => {
  ...
  const choosePlan = async (plan: PlanType) => {
    ...
    if (error || !data?.checkoutUrl) {
      ...
      if (!detail && response) {
        try { detail = (await response.clone().json())?.error; } catch { /* fallback below */ }
      }
  ...
  const description = hasActivePlan && currentPlan
    ...
    : trialEnded
      ? "Seus dados continuam seguros. Escolha um plano para criar orçamentos de obra e gerar PDFs novamente."
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_12%_0%,rgba(245,158,11,0.18),transparent_28rem),linear-gradient(180deg,#fffaf5,#f7f3ef)] px-4 py-6 sm:py-10">
      <div className="mx-auto max-w-5xl">
        <div className="mb-7 flex items-center justify-between gap-3"><BrandMark product="obra" /><span className="rounded-full bg-[#fff0df] px-3 py-1.5 text-xs font-extrabold text-[#a6400d]">OrçaObra</span></div>
          ...
          <div className="p-4 sm:p-7">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {plans.map((plan) => {
                ...
                const cardState = isCurrent ? "ring-2 ring-[#7a5139] ring-offset-2" : isLower ? "opacity-60 grayscale-[0.12]" : "";

                return (
                  <article key={plan.id} className={`relative rounded-[1.35rem] border p-5 sm:p-6 ${cardTone} ${cardState}`}>
                    {isCurrent ? <span className="absolute -top-3 left-4 rounded-full bg-[#6b4a39] px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-wide text-white shadow-sm">Plano atual</span>
                      : plan.highlight && !isLower ? <span className="absolute -top-3 left-4 rounded-full bg-[#e4571f] px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-wide text-white shadow-sm">Oferta temporária</span>
                      : plan.id === "annual" && !isLower ? <span className="absolute -top-3 left-4 rounded-full bg-[#d97706] px-3 py-1 text-[0.66rem] font-extrabold uppercase tracking-wide text-white shadow-sm">12 meses</span> : null}
                    ...
                    <button type="button" disabled={Boolean(loading) || blocked} onClick={() => void choosePlan(plan.id)} className={`mt-5 inline-flex min-h-[3rem] w-full items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-extrabold transition-all disabled:cursor-not-allowed disabled:border-[#ddd5cf] disabled:bg-[#f2efec] disabled:text-[#93877e] disabled:shadow-none ${!blocked ? actionableButton : ""}`}>
            ...
            <p className="mt-5 text-center text-xs leading-5 text-[#74675f]">Os pagamentos são avulsos. O OrçaObra não faz renovação ou cobrança automática ao fim do período.</p>
