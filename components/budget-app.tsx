    ...
    <div className="min-h-screen" style={companyTheme(company)}>
      ...
      {pdfViewer && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#f2f6f5]" role="dialog" aria-modal="true" aria-label={`Visualização de ${pdfViewer.title}`}>
          ...
          <iframe
            src={pdfViewer.url.startsWith("http") ? `https://mozilla.github.io/pdf.js/web/viewer.html?file=${encodeURIComponent(pdfViewer.url)}` : pdfViewer.url}
            title={pdfViewer.title}
            className="min-h-0 flex-1 border-0 bg-[#dfe5e3] md:hidden"
          />
