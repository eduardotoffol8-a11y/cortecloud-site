...
const MIGRATION_KEY = "orcamovel.pwa-migration.v2";
...
export function OrcaMovelPwaMigration() {
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      // Só avisa usuários com evidência da instalação antiga. Navegação normal e
      // instalações novas não recebem o popup.
  ...
  const migrate = async () => {
    ...
    try {
      ...
      if ("caches" in window) {
        ...
        await Promise.all(names.filter((name) => name.startsWith("orcamovel-") && name !== "orcamovel-v15").map((name) => caches.delete(name)));
      }
      if (promptEvent) {
        ...
        setMessage("A nova versão está preparada. No Chrome, toque em ⋮ e escolha “Instalar aplicativo”.");
      }
    } catch {
    ...
    <div className="fixed inset-0 z-[90] grid place-items-end bg-[#102521]/40 p-4 sm:place-items-center" role="dialog" aria-modal="true" aria-labelledby="pwa-migration-title">
      <section className="app-card w-full max-w-md overflow-hidden shadow-2xl">
        <div className="bg-[var(--brand-dark)] p-5 text-white">
          ...
          <h2 id="pwa-migration-title" className="mt-4 text-xl font-extrabold">Nova versão do OrçaMóvel</h2>
          <p className="mt-2 text-sm leading-6 text-[#c8e2de]">Atualizamos a instalação do aplicativo para que o OrçaMóvel funcione de forma independente dos outros aplicativos Orça.</p>
        </div>
        <div className="space-y-4 p-5">
          ...
          <button type="button" disabled={working} onClick={() => void migrate()} className="primary-button w-full"><Download size={18}/>{working ? "Preparando atualização…" : "Atualizar OrçaMóvel"}</button>
