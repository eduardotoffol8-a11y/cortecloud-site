...
import { CalendarClock, Eye, EyeOff, FileText, Fingerprint, KeyRound, LoaderCircle, Mail, Monitor, RefreshCw, ShieldCheck, Smartphone } from "lucide-react";
...
function AuthScreen({ product }: { product: "moveis" | "obra" }) {
  ...
  const signInWithPassword = async (event: React.FormEvent) => {
    event.preventDefault(); if (!supabase) return; setLoading("password"); setError("");
    const result = await supabase.auth.signInWithPassword({ email: email.trim(), password });
    if (result.error) setError("E-mail ou senha incorretos."); setLoading("");
  };

  const signInWithPasskey = async () => {
    if (!supabase) return; setLoading("passkey"); setError("");
    try {
      ...
      if (passkeyError) {
        ...
        else if (passkeyError.name === "NotAllowedError") setError("A entrada foi cancelada ou não há uma credencial cadastrada para este dispositivo.");
        else setError("Não encontramos uma credencial válida. Entre com Google e configure o acesso nos Ajustes.");
      }
    } catch { setError("Este navegador não permitiu usar biometria ou PIN. Entre com Google."); }
    setLoading("");
  };

  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_15%_0%,rgba(15,118,110,0.13),transparent_25rem)] px-4 py-8 sm:grid sm:place-items-center lg:px-8">
        ...
        <div className="mx-auto w-full max-w-md lg:max-w-none">
          ...
          <section className="app-card overflow-hidden">
            ...
            <div className="space-y-4 p-5 sm:p-6">
              ...
              <button type="button" onClick={() => void signInWithPasskey()} disabled={Boolean(loading)} className="secondary-button w-full">{loading === "passkey" ? <LoaderCircle className="animate-spin" size={18} /> : <Fingerprint size={20} />}{loading === "passkey" ? "Aguarde…" : "Entrar com biometria ou PIN"}</button>
              {error && <p role="alert" className="rounded-xl bg-rose-50 px-3.5 py-3 text-sm font-semibold text-rose-700">{error}</p>}
            </div>
            <details className="border-t border-[#e1e9e7] px-5 py-4 sm:px-6"><summary className="cursor-pointer text-center text-sm font-semibold text-[#687875]">Outras formas de entrar</summary><form onSubmit={signInWithPassword} className="mt-4 space-y-3"><label><span className="field-label">E-mail</span><div className="relative"><Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#82918e]" size={18} /><input required type="email" className="field-input !pl-11" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="voce@email.com" autoComplete="email" /></div></label><label><span className="field-label">Senha opcional</span><div className="relative"><KeyRound className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#82918e]" size={18} /><input required type={showPassword ? "text" : "password"} className="field-input !pl-11 !pr-12" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Configurada nos Ajustes" autoComplete="current-password" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-1.5 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-lg text-[#6f7e7b]" aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label><button disabled={Boolean(loading)} className="quiet-button w-full"><KeyRound size={17} />{loading === "password" ? "Aguarde…" : "Entrar com senha"}</button></form></details>
          </section>
...
export function AuthShell({ product = "moveis" }: { product?: "moveis" | "obra" }) {
  ...
  const loadProfile = useCallback(async (currentSession: Session | null) => {
    ...
    if (!currentSession || !supabase) { setProfile(null); setLoading(false); return; }
    ...
    if (data) setProfile({ id: data.id, email: data.email || currentSession.user.email || "", trialEndsAt: data.trial_ends_at, subscriptionStatus: data.subscription_status, planType: data.plan_type, accessExpiresAt: data.access_expires_at });
    setNow(Date.now()); setLoading(false);
  }, [supabase]);

  const refreshCurrentProfile = useCallback(async () => { if (!supabase) return; const { data } = await supabase.auth.getSession(); await loadProfile(data.session); }, [loadProfile, supabase]);

  useEffect(() => {
    if (!supabase) { setLoading(false); return; }
    void refreshCurrentProfile();
    const { data } = supabase.auth.onAuthStateChange((_event, nextSession) => { window.setTimeout(() => void loadProfile(nextSession), 0); });
  ...
  useEffect(() => {
    ...
    window.addEventListener("focus", refreshOnFocus); document.addEventListener("visibilitychange", refreshWhenVisible);
    const params = new URLSearchParams(window.location.search); const paymentReturn = params.get("payment"); const timers: number[] = [];
    if (paymentReturn === "success" || paymentReturn === "pending") { [0, 1200, 3000, 6000, 10000].forEach((delay) => timers.push(window.setTimeout(() => void refreshCurrentProfile(), delay))); window.history.replaceState({}, "", window.location.pathname); }
    return () => { window.removeEventListener("focus", refreshOnFocus); document.removeEventListener("visibilitychange", refreshWhenVisible); timers.forEach((timerId) => window.clearTimeout(timerId)); };
