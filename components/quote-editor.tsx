    ...
    <div className="mb-6 grid grid-cols-3 gap-2" aria-label={`Etapa ${active} de 3`}>
      {([1, 2, 3] as const).map((step) => (
        <button key={step} type="button" onClick={() => onChange(step)} className="text-left">
          <span className={`mb-1.5 block h-1.5 rounded-full transition-colors ${step <= active ? "bg-[#0f766e]" : "bg-[#dbe5e2]"}`} />
          <span className={`text-xs font-bold ${step === active ? "text-[#0f766e]" : "text-[#7b8a87]"}`}>{step}. {labels[step - 1]}</span>
        </button>
      ))}
    </div>
  );
