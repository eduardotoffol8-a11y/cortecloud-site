    ...
    return (
      <section className="view-enter">
        ...
        <div className="app-card p-4 sm:p-6 md:p-7"><div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Apresentação do projeto" className="md:col-span-2"><textarea className="field-input min-h-24 resize-y" value={quote.closing.projectDescription || ""} onChange={(event) => updateClosing("projectDescription", event.target.value)} placeholder="Breve descrição do que será fornecido neste projeto" /><span className="mt-1.5 block text-xs leading-5 text-[#7b8b87]">Este texto abre a proposta e pode ser ajustado para cada projeto.</span></Field>
        </div></div>
    ...
    <section className="view-enter">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="app-card h-fit overflow-hidden lg:sticky lg:top-6">
          <div className="bg-[var(--brand-dark)] px-5 py-5 text-white"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#a9d8d2]">Resumo</p><p className="mt-2 truncate text-lg font-extrabold">{quote.client.name || "Novo orçamento"}</p><p className="mt-1 text-sm text-[#c6e1dd]">{quote.furniture.length} {quote.furniture.length === 1 ? "móvel" : "móveis"}{quote.revision ? ` · Revisão ${String(quote.revision).padStart(2, "0")}` : ""}</p></div>
          <div className="p-5">
