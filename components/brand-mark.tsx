...
export function BrandMark({ compact = false, loading = false }: { compact?: boolean; loading?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <Image src="/pwa-company-icon-192.png" alt="Logo do OrçaMóvel" width={48} height={48} priority className={`h-10 w-10 shrink-0 rounded-xl object-cover shadow-sm ${loading ? "animate-pulse" : ""}`} />
      {!compact && (
        <div>
