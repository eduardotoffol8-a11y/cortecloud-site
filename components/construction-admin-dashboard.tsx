...
type Summary = {
  users:number; trial:number; active:number; monthly:number; annual:number; lifetime:number;
  site_views:number; app_opens:number; workspace_opens:number; site_plan_interests:number;
  plan_opens:number; checkout_starts:number; purchases:number; feedback_total:number;
};
...
export function ConstructionAdminDashboard({onBack}:{onBack:()=>void}){
 ...
 const saveSettings=async()=>{
  ...
  setMessage(error?"Não foi possível salvar os valores.":"Preços e promoção compartilhados atualizados.");
  if(!error)void load();
 };
 const extend=async(user:UserRow)=>{
  if(!supabase)return;
  if(user.subscription_status==="active"&&user.plan_type==="lifetime"){setMessage(`${user.email} já possui acesso vitalício ao OrçaObra.`);return;}
  ...
  const {error}=await supabase.rpc("admin_extend_product_access",{p_user_id:user.id,p_product_id:"obra-civil",p_days:days});
  setMessage(error?"Não foi possível alterar o período.":`${days} dias acrescentados ao acesso do OrçaObra para ${user.email}.`);
  if(!error)void load();
 };
 const feedbackStatus=async(id:number,status:string)=>{if(!supabase)return;const {error}=await supabase.rpc("admin_update_product_feedback_status",{p_id:id,p_product_id:"obra-civil",p_status:status});setMessage(error?"Não foi possível atualizar a avaliação.":"Avaliação atualizada.");if(!error)void load();};
 ...
 return <main className="content-safe mx-auto max-w-6xl px-4 py-6 sm:px-6">
  ...
  <section className="grid grid-cols-2 gap-3 md:grid-cols-5">
   {metric("Usuários",s.users,<Users size={19}/>,"Contas com acesso próprio ao OrçaObra.")}
   {metric("Em teste",s.trial,<Clock3 size={19}/>,"Testes gratuitos do OrçaObra ainda dentro do prazo.")}
   {metric("Planos ativos",s.active,<Check size={19}/>,"Planos pagos do OrçaObra com acesso atualmente válido.")}
   {metric("Compras",s.purchases,<BadgeDollarSign size={19}/>,"Pagamentos aprovados do OrçaObra.")}
   {metric("Avaliações",s.feedback_total,<Star size={19}/>,"Avaliações recebidas no OrçaObra.")}
  </section>

  <section className="mt-6 app-card p-5">
   <h2 className="text-xl font-extrabold">Aquisição e uso</h2>
   <p className="mt-1 text-sm leading-6 text-[#74837f]">Estas métricas são independentes. Uma pessoa pode abrir o OrçaObra por PWA, favorito ou link direto sem passar pela página comercial, por isso o número do app pode ser maior que o do site.</p>
   <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
    ...
    {metric("Abriram o app",s.app_opens,<BarChart3 size={18}/>,"Abriram /apps/obra-civil, inclusive a tela de entrada.")}
    ...
    {metric("Contas cadastradas",s.users,<Users size={18}/>,"Total de acessos próprios criados para o OrçaObra.")}
   </div>
  </section>

  <section className="mt-6 app-card p-5">
   <h2 className="text-xl font-extrabold">Conversão comercial</h2>
   <p className="mt-1 text-sm leading-6 text-[#74837f]">O interesse nos planos da página comercial fica separado da abertura da tela de planos dentro do aplicativo. Assim um clique no site não é confundido com uma intenção de compra já autenticada.</p>
   <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
   ...
   <p className="mt-4 rounded-xl bg-[#f5f8f7] px-4 py-3 text-xs leading-5 text-[#71817d]">A contagem é separada por produto. Ações públicas usam um identificador aleatório do navegador e ações autenticadas usam a conta, evitando misturar OrçaObra com OrçaMóvel.</p>
  </section>

  <section className="mt-6 app-card p-5"><h2 className="text-xl font-extrabold">Preços e promoção</h2><p className="mt-1 text-sm text-[#74837f]">Os valores atuais são compartilhados com o OrçaMóvel. A data é exibida no seu horário local e salva corretamente em UTC.</p><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
  ...
  <section className="mt-6 app-card overflow-hidden"><div className="p-5"><h2 className="text-xl font-extrabold">Usuários e planos</h2><p className="mt-1 text-sm text-[#74837f]">Mensal ativo: {s.monthly} · Anual ativo: {s.annual} · Vitalício: {s.lifetime}</p></div><div className="divide-y divide-[#e4ece9]">{data.users.map(u=><article key={u.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="truncate font-bold">{u.email}</p><p className="mt-1 text-xs text-[#74837f]">{accessLabel(u)} · acesso criado em {new Date(u.created_at).toLocaleDateString("pt-BR")}</p></div><button disabled={u.subscription_status==="active"&&u.plan_type==="lifetime"} onClick={()=>void extend(u)} className="secondary-button !min-h-10 !px-3 disabled:cursor-not-allowed disabled:opacity-50"><Clock3 size={16}/>{u.subscription_status==="active"&&u.plan_type==="lifetime"?"Acesso vitalício":"Adicionar dias"}</button></article>)}</div></section>
 </main>;
}
